...
static float** makeFloatPointersArray(int size){
  return calloc(sizeof(float*), size);
}

static void setFloatPointerAt(float **arr, float *val, int position) {
  arr[position] = val;
}

static void freeFloatPointersArray(float **arr, int size){
  int i;
  for (i = 0; i < size; i++)
    free(arr[i]);
  free(arr);
...
func CreateHist(dims int, numbins int, _type C.int, ranges []float64, uniform bool) *Histogram {
	var hist *C.CvHistogram
	var c_numbins C.int
	var c_range C.float
	var c_uniform C.int
...
func (hist *Histogram) Release() {
	hist_c := (*C.CvHistogram)(hist)
	C.cvReleaseHist(&hist_c)
}
